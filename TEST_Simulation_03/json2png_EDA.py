...
class Json2Png:
    ...
    def __init__(self):
        #리눅스코드 os.chdir("../")
        #리눅스코드 self.path =os.getcwd()

        
        
        self.path =os.path.dirname(os.path.os.getcwd())
        ...
        #json 폴더 경로
        #리눅스코드 self.path_json_dir = self.path + "/토지_피복지도_항공위성_이미지_강원_및_충청/1.라벨링데이터/2.항공사진_Fine_1024픽셀/2.Ground_Truth_JSON_전체/"
        ...
        # 메타데이터 폴더경로
        #리눅스코드 path_meta_dir = self.path + "/토지_피복지도_항공위성_이미지_강원_및_충청/1.라벨링데이터/2.항공사진_Fine_1024픽셀/4.메타데이터/"
    ...
    def make_json2png(self, list_selected, class_color):
        ...
        for i,filename in enumerate(self.list_file_name):
            ...
            with open(json_path,'r', encoding='utf-8') as f02:
                # print(f)
