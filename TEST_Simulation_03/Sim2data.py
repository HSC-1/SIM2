...
from api.detection_person import Api
...
form_class = uic.loadUiType(os.path.dirname(os.path.realpath(__file__))+"\\Sim2data.ui")[0]
...
class WindowClass(QMainWindow, form_class) :
    ...
    def __init__(self) :
        ...
        self.path_yolo_img_dir = os.path.expanduser('~')+"\\pythonQT\\bbox\\img\\"
        self.path_txt_dir = os.path.expanduser('~')+"\\pythonQT\\bbox\\label\\"
        self.save_file_name = os.path.expanduser('~')+"\\pythonQT\\bbox\\new_label\\saving.json"
        ...
        ################################################################################################################
        ## listWidget_Test에 label 데이터 넣기
        # self.listWidget_Test9.clear()
        # for txt_file in yolo2voc_object.list_file_name:
        #      self.listWidget_Test9.addItem(txt_file)
        ################################################################################################################
       ...
       # movie.start()
        ...
        # 이미지 전처리 버튼
        self.pushButton_5.clicked.connect(self.yolococo)
        ...
        self.btn5_connect.clicked.connect(self.connectDet)
        self.btn5_autosave.clicked.connect(self.autoSaveDet)
    ...
    def connectDet(self):
        ...
        if state == True :
            self.label5_01.setText("Connected")
        elif state == False :
            self.label5_01.setText("Not Connected")
    ...
    def groupboxRadFunction(self):
        if self.groupBox_rad_park.isChecked():
            ...
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/AbandonedPark/LinuxNoEditor/AbandonedPark/Binaries/Linux/AbandonedPark'
        elif self.groupBox_rad_africa.isChecked():
            print("groupBox_rad_africa Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/Africa/LinuxNoEditor/Africa_001/Binaries/Linux/Africa_001'
        elif self.groupBox_rad_block.isChecked():
            print("groupBox_rad_block Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/Blocks/LinuxNoEditor/Blocks/Binaries/Linux/Blocks'
        elif self.groupBox_rad_nh.isChecked():
            print("groupBox_rad_nh Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/AirSimNH/LinuxNoEditor/AirSimNH/Binaries/Linux/AirSimNH'
        elif self.groupBox_rad_building.isChecked():
            print("groupBox_rad_building Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/Building99/LinuxNoEditor/Building_99/Binaries/Linux/Building_99'
        elif self.groupBox_rad_mt.isChecked():
            print("groupBox_rad_mt Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/LandscapeMountains/LinuxNoEditor/LandscapeMountains/Binaries/Linux/LandscapeMountains'
        elif self.groupBox_rad_msb.isChecked():
            print("groupBox_rad_msb Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/MSBuild2018/LinuxNoEditor/MSBuild2018/Binaries/Linux/MSBuild2018'
            self.env_path = os.path.expanduser('~')+"\\Documents\\Virtual_envs\WindowsNoEditor\\MSBuild2018.exe"
        elif self.groupBox_rad_trap.isChecked():
            print("groupBox_rad_trap Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/TrapCamera/LinuxNoEditor/TrapCam/Binaries/Linux/TrapCam'
        elif self.groupBox_rad_zhang.isChecked():
            print("groupBox_rad_zhang Checked")
            #리눅스코드 self.env_path = '/home/aiffel-dj16/Downloads/ZhangJiajie/LinuxNoEditor/ZhangJiajie/Binaries/Linux/ZhangJiajie'
    ...
    def clickableBtn2(self):
        #리눅스코드 movie = QMovie("/home/aiffel-dj16/sim2data/pythonQT/image/성능지표_영상.gif")
        movie = QMovie(os.path.os.getcwd()+"\\image\\성능지표_영상.gif")
        print(os.path.os.getcwd()+"\\image\\성능지표_영상.gif")
    ...
    #yolo2voc
    def yolococo(self):
        yolo2voc_object.yolo2coco(self.path_yolo_img_dir, self.path_txt_dir, self.save_file_name, True)
...
if __name__ == "__main__" :
    flag_autosave = 'continue'
   
    def runThread01():
        while(True):
            if flag_autosave =="pause":
                break
            else:
                api.exeDetection()
    ...
    yolo2voc_object = yolo2voc()
    app = QApplication(sys.argv)
    api = Api()
   
    work01 = threading.Thread(target = runThread01,daemon=True)# 데몬스레드
    work02 = threading.Thread(target = runThread02,daemon=True)# 데몬스레드
